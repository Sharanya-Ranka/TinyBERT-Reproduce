...
class DataAugmentor(object):
    ...
    def augment(self, sent, fast=0, mode="slow"):
        ...
        if mode == "fast":
            numbers = np.arange(0, len(tokens))
            random_permutation = np.random.permutation(numbers)
            ...
            for (idx, word) in enumerate(tokens):
                if _is_valid(word) and word.lower() not in StopWordsList:
                    ...
                    if fast == 1:
                        if word in self.past_candidates:
                            candidate_words[idx] = self.past_candidates[word]
                        else:
                            candidates = self._word_augment(sent, idx, word)
                            candidate_words[idx] = candidates
                            self.past_candidates[word] = candidates
    
                    elif fast == 0:
                        candidate_words[idx] = self._word_augment(sent, idx, word)
    
                    else:
                        if np.random.choice([True, False], p=[fast, 1-fast]):
                            if word in self.past_candidates:
                                candidate_words[idx] = self.past_candidates[word]
                            else:
                                candidates = self._word_augment(sent, idx, word)
                            ...
                            if word not in self.past_candidates:
                                self.past_candidates[word] = candidates
                            else:
                                if np.random.choice([True, False], p=[fast, 1-fast]):
                                    self.past_candidates[word] = candidates
...
class AugmentProcessor(object):
    ...
    def read_augment_write(self, fast=0.0, mode="slow"):

        logger.info("Fast augmentations: {}".format(fast))
        logger.info("Augmentations strategy: {}".format(mode))

        task_dir = os.path.join(self.glue_dir, self.task_name)
        train_samples = _read_tsv(os.path.join(task_dir, "train.tsv"))
        output_filename = os.path.join(task_dir, "train_aug_(" + str(mode) + ")_(" + str(fast) + ").tsv")
        ...
        with open(output_filename, 'w', newline='', encoding="utf-8") as f:
            ...
            for (i, line) in enumerate(train_samples):
                ...
                for augment_id in augment_ids_:
                    ...
                    augmented_sents = self.augmentor.augment(sent, fast=fast, mode=mode)
        ...
        # 100: 22.46 -> 20.74 -> 20.28
        # 1000: 212.18 -> 194.38 -> 137.53
        ...
        # 0.75: 3426.34
        # 1.0: 1770.16
        logger.info("Execution time: {} seconds".format(str(end - start)))
...
def main():
    ...
    parser.add_argument("--fast", default=0.0, type=float,
                        help="How often to use dictionary lookup to get the candidate words for augmentation.")
    parser.add_argument("--mode", default="slow", type=str,
                        help="Type of augmention strategy to use.")
    ...
    processor.read_augment_write(fast=args.fast, mode=args.mode)
