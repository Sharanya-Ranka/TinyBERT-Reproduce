...
class MrpcProcessor(DataProcessor):
    ...
    def get_aug_examples(self, data_dir, aug_mode, fast_aug):
        file_name = "train_aug_(" + str(aug_mode) + ")_(" + str(fast_aug) + ").tsv"
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, file_name)), "aug"
        )
...
def main():
    ...
    parser.add_argument("--aug_mode", type=str, default="slow")
    parser.add_argument("--fast_aug", type=float, default=0.0)
    ...
    if not args.do_eval:
        if not args.aug_train:
            ...
            train_examples = processor.get_aug_examples(args.data_dir, args.aug_mode, args.fast_aug)
